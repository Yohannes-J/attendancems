    ...
    <div className="p-4 md:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Manage departments by school</p>
        </div>
        <button
          onClick={openAdd}
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-400">No departments yet.</div>
      ) : (
              ...
              {departments.map((d, i) => (
                <tr key={d._id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-3 text-right space-x-2">
                    <button onClick={() => openEdit(d)} className="text-indigo-600 hover:text-indigo-800 font-medium">Edit</button>
                    <button onClick={() => handleDelete(d._id)} className="text-red-500 hover:text-red-700 font-medium">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
